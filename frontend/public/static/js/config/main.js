...
document.querySelector('.create-config').onclick = () => {
    if (categories.categories.length <= 0) {
        showToastError('No se encontraron categorias.');
        return;
    }
...
document.querySelector('.delete-config').onclick = () => {
    ...
    if (items.length <= 0) {
        showToastError('Selecione al menos una configuración.');
        return;
    }
...
document.querySelector('.export-config').onclick = () => {
    const items = table.getCheckedItems();
    if (items.length <= 0) {
        showToastError('Selecione una configuración.');
        return;
    }
...
list.register(new Observer('append', async item => {
    ...
    try {
        ...
        if (response.status == 201) {
            ...
            showToastSuccess('Configuracion creada con exito!');
            renderApp();
            return;
        }
...
list.register(new Observer('update', async item => {
    ...
    try {
        ...
        if (response.status === 200) {
            showToastSuccess('Configuracion actualizada con exito!');
            return;
        }
        ...
        showToastError('No fue posíbel actualizar la configuración!');
    } finally {
        renderApp();
    }
}));
...
list.register(new Observer('remove', async id => {
    ...
    try {
        ...
        showToastError('Noo fue posible eliminar la configuración!');
    } finally {
        renderApp();
    }
}));
