...
const main = async () => {
    ...
    categoryList.register(new Observer('add', async category => {
        ...
        showToastError(`Error al crear Categoría ${category.name}!`)
    ...
    categoryList.register(new Observer('remove', async category => {
        ...
        try {
            ...
            if (response.status == 204) {
                showToastSuccess(`Categoria ${category.name} eliminada con exito!`)
                return
            }
            ...
            showToastError(`Error al eluminar categoría ${category.name}!`)
        }
    ...
    categoryList.register(new Observer('update', async category => {
        ...
        if (data.status == 200) {
            showToastSuccess(`Categoria ${category.name} actualizado con exito!`)
            return;
        }
    ...
    btnAdd.setOnClick(() => {
        const category = new Category(null, '', '#00000080', 'ACTIVADO', 1)
